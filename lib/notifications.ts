...
export async function addUserToTeamNotifications(
  ...
  try {
    ...
    console.error("Error adding user to team notifications:", error);
    throw error;
  }
}

export async function addUserToTeamNotificationsWithSettings(
  userId: string,
  teamId: string,
  settings: {
  ...
  try {
    // Connect user to team notification settings based on provided settings
    const updateData: any = {};

    if (settings.notificationOnInvitation) {
      updateData.notificationOnInvitationUserIds = { connect: { id: userId } };
    }

    if (settings.notificationOnAssignment) {
      updateData.notificationOnAssignmentUserIds = { connect: { id: userId } };
    }

    if (settings.notificationOnTaskCompletion) {
      updateData.notificationOnTaskCompletionUserIds = {
        connect: { id: userId },
      };
    }

    if (settings.notificationOnCheckin) {
      updateData.notificationOnCheckinUserIds = { connect: { id: userId } };
    }

    if (settings.notificationOnNewTasks) {
      updateData.notificationOnNewTasksUserIds = { connect: { id: userId } };
    }
